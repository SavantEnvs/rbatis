#!/usr/bin/env bash
#
# rbatis/mayhem/build.sh — build rbatis's cargo-fuzz target as a sanitized libFuzzer
...
#
# rbatis is a pure-Rust ORM workspace (rbatis / rbatis-codegen / rbatis-macro-driver).
# The old fork's Mayhem target (`decode`, harness rbdc-pg/fuzz/fuzz_targets/decode.rs) drove
# rbdc-pg's PgValue decode path via in-tree path deps; upstream has since moved the rbdc
# crates out of this repository, so the SAME harness lives additively in mayhem/fuzz/ and
# drives the same entry point through the same-versioned crates.io rbdc-pg + rbs releases
# that rbatis itself depends on.
#
# Also builds the workspace TEST suite (normal flags, no sanitizer) so mayhem/test.sh only RUNS it.
set -euo pipefail
...
# The fuzz crate is ADDITIVE (mayhem/fuzz/) — upstream no longer ships one.
...
# Build the workspace TEST suite with the project's NORMAL flags (no sanitizer RUSTFLAGS) so
# mayhem/test.sh only RUNS the pre-built tests. Same invocation test.sh uses, minus --no-run.
echo "=== building the workspace test suite (normal flags, no sanitizers) ==="
RUSTFLAGS="" cargo test --workspace --no-run --jobs "$MAYHEM_JOBS"

echo "build.sh complete:"
ls -la /mayhem/decode 2>&1 || true
